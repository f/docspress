...
function docspress_customize_register( $wp_customize ) {
	...
	$sections = array(
		...
		'docspress_homepage'   => array(
			...
			'description' => __( 'Choose the front Page in Settings → Reading, then edit that Page to add and customize DocsPress Hero and Audience Paths blocks. Theme controls here only select the template and optional recent-post section.', 'docspress' ),
		),
	...
	$fields       = array(
		...
		'docspress_homepage_layout' => array(
			...
			'description' => __( 'Landing page renders the front Page’s Gutenberg blocks, including optional DocsPress Hero and Audience Paths blocks. Documentation uses the normal three-column Page template.', 'docspress' ),
			'type' => 'select',
			'choices' => array(
